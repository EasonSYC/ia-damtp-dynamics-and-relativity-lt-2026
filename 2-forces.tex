...
\begin{theorem}[Newton's Second Law]
    ...
    \[
        \boxed{\dot{\vb{p}} = \vb{F},}
    \]
    where the \emph{momentum} is defined as \(\vb{p} = m \dot{\vb{x}}\), where \(m\) is the \emph{(inertial) mass} of the particle.
\end{theorem}
...
The \emph{force} \(\vb{F}\) depends on the interactions, but it can \emph{only} depend on \(\vb{x}\) and \(\dot{\vb{x}}\) (and note no \(t\) dependence -- Galilean invariance bans this). In this case, Newton's Second Law is a second-order differential equation, for \(\vb{x} \pqty{t}\). Hence, given \(\vb{x}\) and \(\dot{\vb{x}}\) at \(t = 0\), for all particles, Newton's equations uniquely determine \(\vb{x}\pqty{t}\) for all future times.
...
\[
    \grad V = \pqty{\pdv{V}{x_1}, \pdv{V}{x_2}, \pdv{V}{x_3}}.
\]
...
\begin{definition}[Gravitational Potential Energy]
    \label{def:gpe}%
    The \emph{gravitational potential energy} of a particle of mass \(m\) at \(\vb{x}\), due to a mass \(M\) at \(\vb{x}_0\), is
    \[
        V = - \frac{G M m}{\abs{\vb{x} - \vb{x}_0}}.
    \]
\end{definition}

Here,
\[
    G = \SI{6.67e-11}{\metre\cubed\per\kilogram\per\second\squared}
\]
...
\begin{proposition}[Gravitational Force]
    ...
    \[
        \boxed{\vb{F} = - \grad V = - G M m \frac{\vb{x} - \vb{x}_0}{\abs{\vb{x} - \vb{x}_0}^2}.}
    \]
\end{proposition}
...
\begin{proof}
    ...
    \begin{align*}
        \pdv{x_i} \pqty{\abs{\vb{x} - \vb{x}_0}^2} &= \pdv{x_i} \pqty{\pqty{\vb{x} - \vb{x}_0}_j \pqty{\vb{x} - \vb{x}_0}_j} \\
        &= 2 \pqty{\vb{x} - \vb{x}_0}_j \pdv{x_i} \pqty{\vb{x} - \vb{x}_0}_j \\
        &= 2 \pqty{\vb{x} - \vb{x}_0}_j \kronecker_{ij} \\
        &= \boxed{2 \pqty{\vb{x} - \vb{x}_0}_i,}
    \end{align*}
    while on the other hand
    \[
        \pdv{x_i} \pqty{\abs{\vb{x} - \vb{x}_0}^2} = \boxed{2 \abs{\vb{x} - \vb{x}_0} \pdv{x_i} \abs{\vb{x} - \vb{x}_0}}
    \]

    Hence,
    \[
        \pdv{x_i} \abs{\vb{x} - \vb{x}_0} = \frac{\pqty{\vb{x} - \vb{x}_0}_i}{\abs{\vb{x} - \vb{x}_0}},
    \]
    so
    \[
        \grad \abs{\vb{x} - \vb{x}_0} = \frac{\vb{x} - \vb{x}_0}{\abs{\vb{x} - \vb{x}_0}}.
    \]

    Hence, the gravitational force is given by
    \[
        \vb{F} = - \grad V = - G M m \frac{\vb{x} - \vb{x}_0}{\abs{\vb{x} - \vb{x}_0}^2}.
    \]    
\end{proof}
...
Let \(\vb{r} = \vb{x} - \vb{x}_0\) be the relative position between the two objects. Then
\[
    \vb{F} = - \frac{G M m}{r^2} \unitv{r} = - \frac{G M m \vb{r}}{r^3}
\]
...
Near the surface of the Earth, let \(\vb{x}_0 = \vb{0}\) be the centre of the Earth, and let \(\abs{\vb{x}} = R + z\), where \(R\) is the radius of the Earth, and \(z \ll R\).
...
\begin{align*}
    \Phi\pqty{R + z} &= - \frac{GM}{R + z} \\
    &= - \frac{GM}{R} \pqty{1 - \frac{z}{R} + \frac{z^2}{R^2} - \cdots} \\
    &\approx \text{const.} + \frac{GMz}{R^2}.
\end{align*}

Let
\[
    g = \frac{GM}{R^2} \approx \SI{9.8}{\metre\per\second\squared}.
\]
...
\begin{definition}[Energy due to Conservative Force]
    ...
    \[
        \boxed{E = \frac{1}{2} m \dot{\vb{x}} \vdot \dot{\vb{x}} + V\pqty{\vb{x}}.}
    \]
\end{definition}
...
\begin{proof}
    ...
    \begin{align*}
        \dv{E}{t} &= \frac{1}{2} m \cdot 2 \cdot \ddot{x}_i \dot{x}_i + \pdv{V}{x_i} \dot{x}_i \\
        &= \dot{x}_i \bqty{m \ddot{x}_i + \pdv{V}{x_i}} \\
        &= 0,
    \end{align*}
...
\begin{example}[Object Thrown to Space]
    ...
    To not fall back, the object must reach \(\abs{\vb{x}} = \infty\), without the velocity \(\abs{\dot{\vb{x}}} = 0\). Hence,
    \[
    ...
    and this means
    \[
        v > \boxed{v_{\text{escape}} = \sqrt{\frac{2GM}{R}}} \approx \SI{10}{\kilo\metre\per\second}
    \]
    for the Earth.
\end{example}
...
\begin{definition}[Kinetic Energy]
    ...
    \[
        \boxed{T = \frac{1}{2} m \dot{\vb{x}} \vdot \dot{\vb{x}}.}
    \]
\end{definition}
...
\begin{definition}[Work Done]
    ...
    \[
        \boxed{W = \int_{\mathcal{C}} \vb{F} \vdot \dd{\vb{x}}}
    \]
...
\begin{proof}[using Path Integration and Newton's Second Law]
    \begin{align*}
        W &= \int_{\mathcal{C}} \vb{F} \vdot \dd{\vb{x}} \\
        &= \int_{t_1}^{t_2} \boxed{\vb{F} \vdot \dot{\vb{x}}} \dd{t} \\
        &= m \cdot \int_{t_1}^{t_2} \ddot{\vb{x}} \vdot \dot{\vb{x}} \dd{t} \\
        &= \frac{1}{2} m \int_{t_1}^{t_2} \dv{t} \pqty{\dot{\vb{x}} \vdot \dot{\vb{x}}} \dd{t} \\
        &= T\pqty{t_2} - T\pqty{t_1} \\
        &= V\pqty{t_1} - V\pqty{t_2} && \text{Conservation of Energy} \\
        &= V\pqty{\vb{x}_1} - V\pqty{\vb{x}_2}
    \end{align*}
...
\begin{proof}[using Gradient]
    ...
    \begin{align*}
        W &= \int_{\mathcal{C}} \vb{F} \vdot \dd{\vb{x}} \\
        &= - \int_{\mathcal{C}} \grad V \vdot \dd{\vb{x}} \\
        &= V \pqty{\vb{x}_1} - V \pqty{\vb{x}_2}.
    \end{align*}
\end{proof}
...
Forces that depend on the velocity \(\dot{\vb{x}}\) typically don't have a conserved energy, for example friction, but the \emph{Lorentz Force} is an exception.

\begin{definition}[Electromagnetic Force on Charged Particle]
    \label{def:em-force-charged-particle}%
    The \emph{electromagnetic fields} \(\vb{E}\) and \(\vb{B}\) exert the following forces on a particle with \emph{charge} \(q\)
    \[
        \boxed{\vb{F} = q \pqty{\vb{E} \pqty{\vb{x}} + \dot{\vb{x}} \cross \vb{B} \pqty{\vb{x}}}}.
    \]
\end{definition}
...
\begin{proposition}[Conservation of Energy in Static Electromagnetic Fields]
    ...
    The total energy
    \[
        E = \frac{1}{2} m \dot{\vb{x}} \vdot \dot{\vb{x}} + q \phi\pqty{\vb{x}}
    \]
    is conserved.
\end{proposition}
...
\begin{proof}
    ...
    \begin{align*}
        \dv{E}{t} &= m \ddot{\vb{x}} \vdot \dot{\vb{x}} + q \grad \phi \vdot \dot{\vb{x}} \\
        &= \pqty{\vb{F} + q \grad \phi} \vdot \dot{\vb{x}} \\
        &= \pqty{q \dot{\vb{x}} \cross \vb{B}} \vdot \dot{\vb{x}} \\
        &= 0.
    \end{align*}
\end{proof}
...
Electrical forces are similar to the gravitational ones, that the potential \(\phi\) at \(\vb{x}\) due to another particle of charge \(Q\) at \(\vb{x}_0\) is given by
\[
    \boxed{\phi = \frac{Q}{4 \pi \epsilon_0} \cdot \frac{1}{\abs{\vb{x} - \vb{x}_0}},}
\]
where \(\epsilon_0\) is the permittivity of free space, with
\[
    \epsilon_0 \approx \SI{8.85e-12}{\per\metre\cubed\per\kilogram \second\squared\coulomb\squared}.
\]
...
\[
    m \ddot{\vb{x}} = q \dot{\vb{x}} \cross \vb{B}.
\]

We aim to solve this, which is a \emph{vector differential equation}. The most direct way to solve is to write in Cartesian coordinates. Let \(\vb{B}\) be constant, without loss of generality, in the \(z\)-direction, so
\[
    \vb{B} = \pqty{0, 0, B} = B \unitv{z}.
\]

The equations of motions become, for \(\vb{x} = \pqty{x, y, z}\), that
\begin{align*}
...
\[
    \dot{x} = \tilde{A} \sin \pqty{\omega t + \phi},
\]
where \(\boxed{\omega = \frac{qB}{m}}\) is the \emph{cyclotron frequency}. Hence,
\[
    \boxed{x = x_0 + A \cos \pqty{\omega t + \phi}.}
\]
We put this back into \((1)\), so
\[
    q B \dot{y} = - m A \omega^2 \cos \pqty{\omega t + \phi},
\]
and hence
\[
    \boxed{y = y_0 - A \sin \pqty{\omega t + \phi}.}
\]
...
If the force \(F\pqty{x}\) is independent of the velocity \(\dot{x}\), then it can \emph{always} be written in terms of a potential, by setting
\[
    V\pqty{x} = - \int_{x_0}^{x} \dd{x'} F_x \pqty{x'}
\]
...
\[
    E = \frac{1}{2} m \dot{x}^2 + V\pqty{x}.
\]

Constant energy then gives a first-order differential equation for \(x\pqty{t}\), which is easy to integrate. Indeed,
\[
    E = \frac{1}{2} m \dot{x}^2 + V\pqty{x}
\]
gives
\[
    \dot{x} = \pm \sqrt{\frac{2}{m} \pqty{E - V\pqty{x}}},
\]
and hence integrating gives
\[
    \boxed{t - t_0 = \pm \int_{x_0}^{x} \frac{\dd{x'}}{\sqrt{\frac{2}{m} \pqty{E - V\pqty{x'}}}}}.
\]
...
\begin{definition}[Turning Points]
    The points satisfying  \(V\pqty{x} = E\) (giving \(\dot{x} = 0\)) are called \emph{turning points}.
\end{definition}
...
\begin{definition}[Bounded and Unbounded Motion]
    ...
    \begin{itemize}
        ...
        \item Region (b) gives \emph{unbounded motion}, since the particle bounces off the turning point, and flies off to \(x \to \minusinfty\).
    \end{itemize}
\end{definition}
...
\begin{definition}[Equilibrium Points]
    ...
    \emph{Equilibrium points} are when \(V\pqty{x} = E\) and \(V'\pqty{x} = 0\).
\end{definition}
...
\begin{align*}
    V\pqty{x} &\approx V\pqty{x_0} + \pqty{x - x_0} V' \pqty{x_0} + \frac{1}{2} \pqty{x - x_0}^2 V''\pqty{x_0} \\
    &= V\pqty{x_0} + \frac{1}{2} \pqty{x - x_0}^2 V''\pqty{x_0}.
\end{align*}
...
\begin{definition}[Stable and Unstable Equilibrium Points]
    ...
    \begin{itemize}
        \item If \(\boxed{V''\pqty{x_0} > 0}\), we say \(x_0\) is a \emph{stable equilibrium point} where \(V\) is a \emph{minimum}.
        \item If \(\boxed{V''\pqty{x_0} < 0}\), we say \(x_0\) is an \emph{unstable equilibrium point} where \(V\) is a \emph{maximum}.
    \end{itemize}
...
\[
    V'\pqty{x} \approx V'\pqty{x_0} + \pqty{x - x_0} V''\pqty{x_0} = \pqty{x - x_0} V''\pqty{x_0}.
\]

Hence, Newton's Equations gives
\[
    m \ddot{x} = - V'\pqty{x} \approx - \pqty{x - x_0} V''\pqty{x},
\]
and hence
\[
    x = x_0 + A \cos \pqty{\omega t + \phi},
...
\[
    x = x_0 + \tilde{A} \exp \pqty{\gamma t} + \tilde{B} \exp \pqty{- \gamma t}
\]
where
\[
    \gamma = \sqrt{- \frac{V''\pqty{x_0}}{m}}.
\]
...
If \(V''\pqty{x_0} = 0\), we will have to expand to a higher order term.
...
\begin{example}[Motivating Dimensional Analysis for a Pendulum]
    ...
    Furthermore, let
    \[
        \theta\pqty{t} = \theta\pqty{t\pqty{\tau}} = \theta\pqty{\tau}.
    \]
    ...
    The solution of this equation is independent of \(g\) and \(l\). Therefore, the solution is some \(\theta\pqty{\tau}\). The period \(\Delta \tau\) of \(\theta\pqty{\tau}\) may depend on the initial angle \(\theta_0 = \theta\pqty{-}\), but cannot depend on \(g\) and \(l\). Hence,
    \[
        \theta\pqty{\tau + \Delta \tau} = \theta\pqty{\tau}
    \]
    gives
    \[
...
Then we have
\begin{align*}
    \bqty{\dot{x}} &= \dimL \dimT\inverse, \\
    \bqty{\ddot{x}} &= \dimL \dimT^{-2}, \\
    \bqty{F} &= \dimM \dimL \dimT^{-2}, \\
    \bqty{E} &= \dimM \dimL^2 \dimT^{-2},
\end{align*}
etc.
...
All arguments of non-trivial functions (i.e. involving sums of different powers) must be \emph{dimensionless}, e.g., \(\sin \theta\) tells us that \(\bqty{\theta} = \dimLess\).
...
\begin{example}[Dimensional Analysis on Pendulum]
    ...
    The initial angle satisfies \(\bqty{\theta_0} = \dimLess\). These are all the quantities involved, and we want to find the period \(\bqty{T} = \dimT\). We let
    \[
        T = f\pqty{\theta_0} g^A l^B m^C.
    \]

    Taking dimensions, we have
    \[
        \dimT = \bqty{T} = \bqty{g}^A \bqty{l}^B \bqty{m}^C = \dimL^A \dimT^{-2A} \dimL^{B} \dimM^{c}.
    \]

    This solves to \(A = - \frac{1}{2}\), \(B = \frac{1}{2}\), and \(C = 0\). So
    \[
        T = f\pqty{\theta_0} \sqrt{\frac{l}{g}}.
    \]
\end{example}
...
For the second property, if ever in doubt of the sign of a friction force, it must always \emph{slow an object down}. This means that the friction force must change sign if velocity is reversed from \(\vb{v}\) to \(- \vb{v}\).
...
\begin{definition}[Linear and Quadratic Drag]
    ...
    \begin{itemize}
        ...
        \[
            \boxed{\vb{F} = - k_2 \abs{\vb{v}} \vb{v} = - k_2 \pqty{\vb{v} \vdot \vb{v}} \unitv{v}.}
        \]
    \end{itemize}
...
\[
    \bqty{k_2 v^2} = \dimM \dimL^{-3} \dimL^2 \dimL^2 \dimT^{-2} = \dimM \dimL \dimT^{-2}.
\]
...
Heavier objects reach a bigger terminal velocity, and for example, raindrops have a terminal velocity of \(\SI{10}{\metre\per\second}\). 
...
\begin{example}[Timescale to reach Terminal Velocity]
    ...
    and hence
    \[
        \bqty{t_0} = \dimT = \dimM^A \dimL^B \dimT^{-2B} \dimM^C \dimL^{-C} = \dimM^{A + C} \dimL^{B - C} \dimT^{-2B}.
    \]
...
For raindrops, \(t_0 \approx \SI{1}{\second}\). However, note that when we solve the equation, the graph of \(v\) against \(t\) looks something like follows.
...
\[
    m \dv{\vb{v}}{t} = m \vb{g} - k \abs{\vb{v}} \vb{v},
\]
so it is natural to treat this as a vector differential equation for \(\vb{v}\).
...
\[
    \boxed{x = \exp\pqty{- \alpha t} \bqty{A_+ \exp\pqty{i \Omega t} + A_- \exp\pqty{- i \Omega t}},}
\]
...
\begin{itemize}
    ...
    \[
        \boxed{x = \pqty{A + Bt} \exp\pqty{-\alpha t}.}
    \]
\end{itemize}
