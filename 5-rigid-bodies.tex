...
\begin{definition}[Angular Velocity]
    ...
    The \emph{angular velocity}, \(\vb*{\omega}\), is defined as
    \[
        \vb*{\omega} = \omega \unitv{n},
    \]
    where
    \begin{itemize}
        \item \(\unitv{n}\) points along the axis of rotation; and
        \item \(\omega = \abs{\vb*{\omega}} = \dot{\theta}\) is the angular speed of rotation.
    \end{itemize}
\end{definition}
...
\begin{lemma}[Cross Product Relation between Velocity and Angular Velocity]
    ...
    We have
    \[
        \boxed{\dot{\vb{x}} = \vb*{\omega} \cross \vb{x}.}
    \]
\end{lemma}
...
\begin{itemize}
    \item \(\dot{\vb{x}}\) is orthogonal to both \(\vb*{\omega}\) and \(\vb{x}\); and
    \item \(\abs{\dot{\vb{x}}} = \omega \abs{\vb{x}} \sin \phi = \omega d\).
\end{itemize}

Indeed, we have \(\omega = \abs{\dot{\theta}}\), and \(d = \abs{\unitv{n} \cross \vb{x}}\).
...
\begin{lemma}[All Particles have Same Angular Velocity for Rigid Body]
    ...
    \[
        \dot{\vb{x}}_i = \vb*{\omega} \cross \vb{x}_i.
    \]
\end{lemma}
...
\begin{proof}
    ...
    \begin{align*}
        \dv{t} \abs{\vb{x}_i - \vb{x}_j}^2 &= 2 \pqty{\dot{\vb{x}}_i - \dot{\vb{x}}_j} \vdot \pqty{\vb{x}_i - \vb{x}_j} \\
        &= 2 \bqty{\vb*{\omega} \cross \pqty{\vb{x}_i - \vb{x}_j}} \vdot \pqty{\vb{x}_i - \vb{x}_j} \\
        &= 0.
    \end{align*}
\end{proof}
...
\begin{proposition}[Kinetic Energy of Rotating Rigid Body]
    ...
    \[
        \boxed{T = \frac{1}{2} \sum_{i = 1}^{n} m_i \pqty{\dot{\vb{x}}_i \vdot \dot{\vb{x}}_i} = \frac{1}{2} I \omega^2.}
    \]
\end{proposition}
...
\begin{proof}    
    ...
    \begin{align*}
        T &= \frac{1}{2} m \dot{\vb{x}} \vdot \dot{\vb{x}} \\
        &= \frac{1}{2} m \pqty{\vb*{\omega} \cross \vb{x}} \vdot \pqty{\vb*{\omega} \cross \vb{x}} \\
...
\begin{proposition}[Angular Momentum for Rigid Body]
    ...
    \[
        \vb{L} = \sum_{i = 1}^{n} m_i \pqty{\vb{x}_i \cross \dot{\vb{x}}_i},
    \]
...
\begin{proof}
    ...
    \begin{align*}
        L &= \vb{L} \vdot \unitv{n} \\
        &= \omega \sum_{i = 1}^{n} m_i \bqty{\vb{x}_i \cross \pqty{\unitv{n} \cross \vb{x}_i}} \vdot \unitv{n} \\
        &= \omega \sum_{i = 1}^{n} m_i \bqty{\pqty{\unitv{n} \cross \vb{x}_i} \vdot \pqty{\unitv{n} \cross \vb{x}_i}} \\
...
\begin{proof}
    Recall that we have \(\dot{\vb{L}} = \vb{G}\). Dotting this with \(\unitv{n}\), we have
    \[
        G = \dot{\vb{L}} \vdot \unitv{n} = I \dot{\omega}
    \]
    as desired.
\end{proof}
...
\[
    T = \frac{1}{2} \vb*{\omega}\transpose  \tens{I}  \vb*{\omega},
\]
where the inertia tensor \(\tens{I}\) is defined as
\[
    I_{ab} = \sum_{i = 1}^{n} m_i \bqty{\pqty{\vb{x}_i \vdot \vb{x}_i} \kronecker_{ab} - \pqty{\vb{x}_i}_a \pqty{\vb{x}_i}_b}.
\]
...
\[
    \sum_{i = 1}^{n} m_i f\pqty{\vb{x}_i} = \iiint_{V} \dd[3]{\vb{x}} \rho\pqty{\vb{x}} f\pqty{\vb{x}},
\]
for some function \(f\pqty{\vb{x}}\), for example, \(d_i^2\), where \(\rho\pqty{\vb{x}}\) is the density. Therefore,
\[
    M = \sum_{i = 1}^{n} m_i = \iiint_V \dd[3]{\vb{x}} \rho\pqty{\vb{x}},
\]
and we have
\[
    I = \iiint_V \dd[3]{\vb{x}} \rho\pqty{\vb{x}} x_{\perp}^2.
\]
...
\begin{example}[Rod, Rotating about its End]
    ...
    \[
        I = \rho \int_{0}^{l} \dd{x} \cdot x^2 = \frac{\rho l^3}{3}.
    \]
...
\begin{example}[Disc/2-Ball, Rotating about its Axis of Symmetry]
    ...
    \begin{align*}
        I &= \rho \cdot 2\pi \cdot \int_{0}^{a} r \dd{r} \cdot r^2 \\
...
\begin{example}[Ball/3-Ball, Rotating about a Diameter]
    ...
    \begin{align*}
        I &= \rho \cdot 2\pi \cdot \int_{0}^{a} r^2 \dd{r} r^2 \cdot \int_{0}^{\pi} \sin \theta \dd{\theta} \sin^2 \theta \\
        &= \rho \cdot 2\pi \cdot \pqty{\frac{a^5}{5}} \cdot \pqty{2 \cdot \frac{2}{3}} \\
...
\begin{proof}
    ...
    \begin{align*}
        I_x &= \iint \dd[2]{\vb{x}} \rho y^2, \\
        I_y &= \iint \dd[2]{\vb{x}} \rho x^2, \\
        I_z &= \iint \dd[2]{\vb{x}} \rho \pqty{x^2 + y^2}.
    \end{align*}
...
\begin{proof}
    ...
    \]
    and we recall that \(\sum_{i = 1}^{n} \vb{y}_i m_i = \vb{0}\).

    Then, we have
    \begin{align*}
        I &= \sum_{i = 1}^{n} m_i \pqty{\unitv{n} \cross \vb{x}_i} \vdot \pqty{\unitv{n} \cross \vb{x}_i} \\
        &= \sum_{i = 1}^{n} m_i \bqty{\unitv{n} \cross \pqty{\vb{R} + \vb{y}_i}} \vdot\bqty{\unitv{n} \cross \pqty{\vb{R} + \vb{y}_i}} \\
        &= \sum_{i = 1}^{n} m_i \bqty{\pqty{\unitv{n} \cross \vb{R}} \vdot \pqty{\unitv{n} \cross \vb{R}} + \pqty{\unitv{n} \cross \vb{y}_i} \vdot \pqty{\unitv{n} \cross \vb{y}_i} + 2 \pqty{\unitv{n} \cross \vb{R}} \vdot \pqty{\unitv{n} \cross \vb{y}_i}}.
    \end{align*}

    Note that the first term has \(\abs{\unitv{n} \cross \vb{R}} = h\), and hence gives the \(M h^2\) term; the second term is precisely the moment of inertia about the axis through the centre of mass, giving the \(I_{\text{c.o.m.}}\) term; the final term reduces to \(0\) since the sum of \(m_i \vb{y}_i\) is \(\vb{0}\).
...
\[
    \vb{x}_i = \vb{R} \pqty{t} + \vb{y}_i,
\]
and let the angular velocity \(\vb*{\omega}\) capture the rotation about its centre of mass, in the form
\[
    \dot{\vb{y}}_i = \vb*{\omega} \cross \vb{y}_i.
\]
...
\begin{proposition}[Rotational Kinetic Energy]
    ...
    \[
        \boxed{T = \frac{1}{2} M \dot{\vb{R}} \vdot \dot{\vb{R}} + \frac{1}{2} I_{\text{c.o.m.}} \omega^2}.
    \]
\end{proposition}
...
\begin{proof}
    ...
    \[
        T = \frac{1}{2} M \dot{\vb{R}} \vdot \dot{\vb{R}} + \frac{1}{2} \sum_{i = 1}^{n} m_i \dot{\vb{y}}_i \vdot \dot{\vb{y}}_i.
    \]
...
\[
    V = \sum_{i = 1}^{n} V_i \pqty{\vb{x}_i} + \sum_{i < j} V_{ij} \pqty{\abs{\vb{x}_i - \vb{x}_j}}.
\]
...
\begin{example}[Rigid Rod Pendulum]
    ...
    \[
        T = \frac{1}{2} M v_{\text{c.o.m.}}^2 + \frac{1}{2} I_{\text{c.o.m.}} \pqty{\dot{\theta}}^2.
    \]
    ...
    \begin{align*}
        T &= \frac{1}{2} M \frac{L^2}{4} \pqty{\dot{\theta}}^2 + \frac{1}{2} \pqty{I - M \frac{L^4}{4}} \pqty{\dot{\theta}}^2 \\
        &= \frac{1}{2} I \pqty{\dot{\theta}}^2,
    \end{align*}
    which agrees with the previous result.

    The total energy, including the potential component, is then given by
    \[
        E = \frac{1}{2} I \pqty{\dot{\theta}}^2 - Mg \frac{L}{2} \cos \theta,
    \]
...
\begin{example}[Sliding/Rolling Ball]
    ...
    \[
        T = \frac{1}{2} m v^2 + \frac{1}{2} I \omega^2 = \frac{1}{2} \pqty{m + \frac{I}{a^2}} v^2,
    \]
    ...
    \begin{align*}
        \dv{E}{t} &= M \dot{x} \ddot{x} + I \dot{\theta} \ddot{\theta} \\
        &= \dot{x} \pqty{-f} + \pqty{\dot{\theta} a} f \\
        &= f\pqty{- \dot{x} + \dot{\theta} a} \\
        &= 0
    \end{align*}
    ...
    \[
        E = \frac{1}{2} \pqty{M + \frac{I}{a^2}} \pqty{\dot{x}}^2 - Mg x \sin \alpha.
    \]

    Imposing the conservation of energy, we have
    \[
        \boxed{\pqty{M + \frac{I}{a^2}} \ddot{x} = Mg \sin \alpha.}
    \]
...
\begin{example}[Elastic Bounce of Ball against Earth]
    ...
    \[
        \frac{1}{2m} \pqty{q_y^2 + Q_y^2 + 2 q_y Q_y} = \frac{q_y^2}{2m} + \frac{Q_y^2}{2M}.
    \]
...
\begin{definition}[Impulse]
    \label{def:impulse}%
    The change in momentum over a short time, \(\Delta \vb{p}\), is called an \emph{impulse}, denoted \(\vb{I}\).
\end{definition}