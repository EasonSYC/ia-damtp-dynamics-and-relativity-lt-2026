...
\begin{definition}[Point Particle, Position, Velocity, Acceleration]
    ...
    A \emph{point particle} is an idealised object that is completely determined by its \emph{position} at a given time, \(\vb{x} \pqty{t}\).

    The \emph{velocity} is the time derivative of the position, denoted as
    \[
        \boxed{\vb{v} = \dv{\vb{x}}{t} = \dot{\vb{x}}.}
    \]

    The \emph{acceleration} is defined as
    \[
        \boxed{\vb{a} = \ddot{\vb{x}} = \dot{\vb{v}} = \dv[2]{\vb{x}}{t}.}
    \]
\end{definition}
...
\[
    \vb{v} = \dv{\vb{x}}{t} = \pqty{\dv{x_1}{t}, \dv{x_2}{t}, \dv{x_3}{t}},
\]
...
\begin{definition}[Galilean Transformation]
    ...
    \[
        \boxed{\vb{x}' = \vb{R} \vb{x} + \vb{k} + \vb*{\omega} t,}
    \]
    where:
    \begin{itemize}
        \item \(\vb{R} \in \Or\pqty{3}\) is a constant \emph{rotation} or a \emph{reflection} matrix, i.e., \(\vb{R} \transpose \vb{R} = \identityM\);
        \item \(\vb{k}\) is a constant vector that defines \emph{translation}; and
        \item \(\vb*{\omega}\) is a constant velocity called a \emph{boost}.
    \end{itemize}
\end{definition}

Mathematically, it is easy to see that
\[
    \ddot{\vb{x}} = \vb{0} \iff \ddot{\vb{x}}' = \vb{0}.
\]
