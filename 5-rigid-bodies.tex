...
\begin{proof}
    ...
    \begin{align*}
        \DiffOp{t} \abs{\vm{x}_i - \vm{x}_j}^2 &= 2 \para{\dot{\vm{x}}_i - \dot{\vm{x}}_j} \cdot \para{\vm{x}_i - \vm{x}_j} \\
...
\[
    V = \sum_{i = 1}^{n} V_i \para{\vm{x}_i} + \sum_{i < j} V_{ij} \para{\abs{\vm{x}_i - \vm{x}_j}}.
\]
